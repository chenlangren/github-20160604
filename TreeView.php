...
HTML;
    ...
    */
    public function initQuery(){
        ...
        $this->pagenation = new Pagination(['totalCount' =>  $this->query->where("lvl=0")->count()]);
        $this->pagenation->defaultPageSize = 5;
        
        $sqlstr = <<<EOF
SELECT id FROM plan WHERE lvl=0
ORDER BY id
LIMIT {$this->pagenation->offset}, {$this->pagenation->defaultPageSize}
EOF;
        
        $rootIds = Yii::$app->db->createCommand($sqlstr)
            ->queryAll();
        $strIds = "(";
        foreach ($rootIds as $value) {
            $strIds .= $value['id'] . ",";
        }
        $strIds = substr($strIds, 0, -1);
        $strIds .= ")";
//        $strIds = implode(",",$rootIds);
        $this->_nodes = $this->query
                ->where("root IN {$strIds}")
                ->all();   
    }
...
//        $this->_nodes = $this->query->all();
        ...
        $defaultToolbar = [
            self::BTN_CREATE => [
                'icon' => 'plus',
                'options' => ['title' => Yii::t('kvtree', 'Add place'), 'disabled' => true]
            ],
            self::BTN_CREATE_ROOT => [
                'icon' => $this->fontAwesome ? 'tree' : 'tree-conifer',
                'options' => ['title' => Yii::t('kvtree', 'Add new trip')]
            ],
            self::BTN_REMOVE => [
                'icon' => 'trash',
...
//            $nodeIconType = $node->$iconTypeAttribute;
            ...
            $out .= Html::beginTag('li', $nodeOptions) . "\n" .
                ...
                $this->renderNodeIcon(null, null, $isChild) . "\n" .
